...
- [x] 7.1 Прогнать `dart run melos run codegen` (`codegen.cmd`) и сверить `*.g.dart` — результат: `power_training.g.dart` совпал с ручной версией (build_runner его не перезаписал), `exercise.g.dart`, `exercise_set.g.dart` и `database.g.dart` перегенерированы (в `database.g.dart` вернулись менеджеры и композеры таблиц), расхождений в рабочем дереве нет
- [x] 7.2 Прогнать `flutter analyze` — результат: ошибок и предупреждений нет
- [x] 7.3 Прогнать `flutter gen-l10n` (`generateLocalizations.cmd`) — результат: ключи на месте, сгенерированные файлы совпадают с ручными версиями
- [x] 7.4 Переустановить приложение на устройстве (или очистить данные приложения) и убедиться, что схема создаётся с нуля — результат: приложение запускается на чистой базе, старые локальные данные не мешают
- [x] 7.5 Проверить на устройстве борд 04 → кнопку → состояние борда 13 на экране тренировки → возврат назад — результат: навигация работает в обе стороны, экран без набора выглядит как макет
